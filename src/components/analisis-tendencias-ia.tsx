...
// Panel "Análisis de tendencias con IA" para la pantalla de Reportes. Llama al
// endpoint propio (POST /api/ia/analisis-tendencias) que agrega las series de
// tiempo del negocio (según rol) y las pasa a Claude, mostrando el texto en
// streaming. Comparte el cupo mensual con las demás funciones de IA.
export function AnalisisTendenciasIA() {
  ...
  async function generar() {
    ...
    setAbierto(true);
    try {
      const res = await fetch("/api/ia/analisis-tendencias", { method: "POST" });
      if (!res.ok || !res.body) {
        const d = await res.json().catch(() => ({}));
        setError(d.error ?? "No se pudo generar el análisis.");
        setCargando(false);
        return;
      }
      ...
      setError("No se pudo generar el análisis. Revisa tu conexión e inténtalo de nuevo.");
    } finally {
      setCargando(false);
      cargarUso();
    }
    ...
    <div className="mb-6 rounded-2xl border border-brand-200 bg-gradient-to-br from-brand-50 via-white to-brand-100/60 p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-sm font-bold text-slate-800">Análisis de tendencias con IA</h2>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Lee tus series de los últimos meses y explica qué cambió, por qué y qué hacer.
            </p>
          </div>
        </div>
        <button
        ...
        >
          ...
          <span>{cargando ? "Generando…" : sinPlan ? "No disponible" : topeAlcanzado ? "Límite alcanzado" : texto || error ? "Regenerar" : "Generar análisis"}</span>
        </button>
      </div>
