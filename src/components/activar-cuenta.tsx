...
export function ActivarCuenta({
  ...
  const passwordOk = !necesitaPassword || (nueva.length >= 8 && nueva === confirmar);
  const terminosOk = !necesitaTerminos || acepto;
  const puedeEnviar = passwordOk && terminosOk && !enviando;

  async function handleActivar() {
    setError("");
    if (necesitaPassword && nueva.length < 8) { setError("La contraseña debe tener al menos 8 caracteres."); return; }
    if (necesitaPassword && nueva !== confirmar) { setError("Las contraseñas no coinciden."); return; }
    if (necesitaTerminos && !acepto) { setError("Debes aceptar el Acuerdo de Licencia."); return; }
    ...
    try {
      const res = await fetch("/api/activar-cuenta", {
        ...
        body: JSON.stringify({ nuevaPassword: necesitaPassword ? nueva : undefined, acepto }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        setError(data.error ?? "No se pudo activar la cuenta. Intenta de nuevo.");
        setEnviando(false);
        return;
      }
    } catch {
      setError("No se pudo activar la cuenta. Revisa tu conexión e intenta de nuevo.");
      setEnviando(false);
      return;
    }
    // Recarga completa para que el gate del servidor consulte la DB actualizada.
    ...
    <div className="min-h-screen bg-slate-50 flex items-start justify-center py-10 px-4">
      <div className="w-full max-w-3xl">
        ...
        <div className="bg-brand-950 rounded-2xl px-8 py-6 mb-6 text-center">
          ...
          <h1 className="text-white text-2xl font-bold">Activa tu cuenta</h1>
          <p className="text-brand-300 text-sm mt-1">
            {necesitaPassword && necesitaTerminos
              ? "Define tu contraseña y acepta el acuerdo para continuar"
              : necesitaPassword
                ? "Define una contraseña personal para continuar"
                : "Acepta el acuerdo para continuar"}
          </p>
        </div>
        ...
        {necesitaPassword && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6">
            ...
            <p className="text-xs text-slate-400 mb-4">
              La clave que te compartieron es temporal. Elige una que solo tú conozcas (mínimo 8 caracteres).
            </p>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">Nueva contraseña</label>
                <CampoPassword value={nueva} onChange={e => setNueva(e.target.value)} placeholder="Mínimo 8 caracteres"
              ...
              <div>
                ...
                <CampoPassword value={confirmar} onChange={e => setConfirmar(e.target.value)} placeholder="Repite la contraseña"
                  className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
              </div>
        ...
        {necesitaTerminos && (
          <>
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6">
              ...
              <div className="h-[460px] overflow-y-auto px-8 py-6 text-sm text-slate-700 leading-relaxed space-y-4">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800 font-medium flex items-center gap-2">
                  ...
                  LEA ESTE ACUERDO DETENIDAMENTE. AL ACEPTARLO, VINCULA LEGALMENTE A SU EMPRESA A ESTOS TÉRMINOS.
                </div>
              ...
              <div className="border-t border-slate-100 px-8 py-2 bg-slate-50">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-sm text-slate-700">
                  He leído y entendido el <strong>Acuerdo de Licencia de Uso versión {VERSION}</strong> en su totalidad, tengo capacidad legal para aceptarlo en nombre de mi empresa, y acepto quedar vinculado por todos sus términos y condiciones.
                </span>
              </label>
            </div>
          </>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="font-bold text-slate-900 text-base mb-2 border-b border-slate-100 pb-1">{title}</h3>
