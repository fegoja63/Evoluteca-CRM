...
type KpiCardProps = {
  ...
  iconBg?: string;
  iconColor?: string;
  /** @deprecated usa `icon` (Tabler) en vez de un emoji suelto */
  emoji?: string;
  icon?: Icon;
};
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col justify-between min-h-[110px] relative overflow-hidden">
      <div className="flex items-start justify-between">
        ...
        <div className={`w-11 h-11 ${iconBg ?? "bg-brand-50"} rounded-xl flex items-center justify-center text-xl`}>
          {Icono ? <Icono size={20} stroke={1.75} className={iconColor ?? "text-brand-600"} /> : emoji}
        </div>
      </div>
