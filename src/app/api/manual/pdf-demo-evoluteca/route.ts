...
export async function GET() {
  ...
  const doc = React.createElement(Document,
    ...
    React.createElement(Page, { size: "A4", style: s.page },
      ...
      React.createElement(P, null, "Esta cuenta se dejó preparada a propósito con ejemplos reales de las funciones más recientes del CRM, para que las veas en acción con datos concretos, no solo leerlas."),
      ...
      React.createElement(Paso, { n: 12, titulo: "Resumen con IA del cliente", desc: "En Clientes, abre una cuenta con historial (por ejemplo 'Tech Solutions Colombia') y en el panel 'Resumen con IA' haz clic en 'Generar resumen'. En segundos verás un informe de seis secciones (panorama de la cuenta, relación y actividad, oportunidades y cotizaciones, señales, contactos clave y próximas acciones) redactado con los datos reales de esa cuenta." }),
      React.createElement(Paso, { n: 13, titulo: "Brief del pipeline con IA", desc: "Ve a Pipeline; arriba, sobre los filtros, verás el panel 'Brief del pipeline con IA'. Haz clic en 'Generar brief' para obtener en segundos un resumen ejecutivo de todo el pipeline: panorama, negocios calientes, en riesgo, avance de la meta y prioridades de la semana." }),
      React.createElement(Nota, null, "Estas funciones se sumaron para resolver necesidades reales detectadas en cuentas ya operando: visibilidad por vendedor, aprender de por qué se pierde un negocio, trazabilidad documental, captura automática de leads externos, un pipeline que se adapta al vocabulario de cada negocio, y dos asistentes de IA que preparan el resumen de una cuenta y el brief del pipeline en segundos. Las funciones de IA comparten un cupo mensual y solo usan los datos reales de tu CRM."),
    ),
