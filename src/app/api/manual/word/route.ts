  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        p("Debajo, la misma tarjeta incluye una sección \"Próximas actividades\" con las siguientes 5 actividades pendientes desde mañana en adelante (título, empresa o contacto y fecha), para ver lo que viene sin salir del Dashboard. Si no hay actividades futuras pendientes, muestra el aviso \"No hay actividades próximas programadas\"."),
        ...
        p("Ve a Clientes → botón \"+ Nuevo cliente\". El \"Nombre del cliente o Empresa\" es obligatorio. Puedes escribir el \"Nombre del contacto\" (contacto principal, que usa el email y teléfono ingresados arriba) y, en \"OTRO contacto de este cliente\", un segundo contacto — así el cliente queda con más de un contacto desde el inicio. Los demás campos son opcionales pero recomendados."),
        tip("El sector ayuda a filtrar y segmentar clientes. Elige el más cercano a la actividad del cliente."),
        p("Clientes nuevos por año o mes: junto al buscador de la lista de Clientes puedes filtrar por \"Creados en …\" (año) y, si quieres, por mes. La lista, las tarjetas de resumen y el botón Exportar Excel se ajustan a ese período — útil para distinguir los clientes nuevos del año de los de años anteriores."),
        ...
        bullet("Contactos vinculados: agrégalos con \"+ Agregar contacto\", edítalos o elimínalos con el ícono de lápiz, y escríbeles por WhatsApp si tienen teléfono"),
        ...
        p("Los contactos son personas dentro de un cliente. Puedes crearlos, editarlos y eliminarlos desde la ficha del cliente (sección Contactos, botón \"+ Agregar contacto\" y el ícono de lápiz de cada uno) o desde la página Contactos del menú. Un cliente puede tener varios contactos. Si el contacto tiene teléfono registrado, aparece el botón 💬 WhatsApp que abre la conversación directamente con ese número."),
        ...
        h2("4.4 Indicadores de estancamiento"),
        p("Cada tarjeta del kanban muestra un borde de color a la izquierda según hace cuánto la oportunidad no tiene movimiento — sin actividad registrada ni cambio de etapa (no la simple antigüedad desde que se creó): verde (al día), ámbar (estancada: alcanzó el umbral de días que definió tu Administrador) y rojo (muy estancada: el doble del umbral). El badge muestra los días sin movimiento (ej: \"22d\"). Las oportunidades Ganadas o Perdidas no muestran indicador. El umbral (por defecto 14 días) se ajusta en Configuración → Alerta de negocios estancados. Cuando hay estancadas, un chip rojo \"N estancadas\" en los filtros permite verlas solas."),
        ...
        p("Al elegir una Empresa en el formulario, los campos Contacto y Oportunidad se filtran automáticamente para mostrar solo los que pertenecen a esa empresa. Si cambias de empresa y el contacto u oportunidad seleccionado ya no corresponde, el campo se limpia solo. Sin empresa seleccionada, ambos campos muestran todas las opciones."),
        p("Si el contacto todavía no existe, el botón \"+ Nuevo contacto\" junto al campo Contacto permite crearlo al instante: escribe el nombre y pulsa Agregar (o Enter); el contacto se crea, queda asignado a la empresa seleccionada y se selecciona en la actividad, sin salir de la Agenda."),
        nota("Los contactos sin empresa asignada no aparecen en el campo Contacto cuando hay una empresa seleccionada; asígnales primero su empresa desde la ficha del contacto."),
        ...
        bullet("Negocios estancados: oportunidades activas sin actividad ni cambio de etapa desde hace más del umbral de días configurado (por defecto 14)"),
        ...
        h3("5.4.1 Resumen del Lunes (correo semanal)"),
        p("Además de las alertas diarias, cada lunes en la mañana el CRM envía el \"Resumen del Lunes\": un solo correo por persona que abre la semana con una cabecera de indicadores (pipeline activo, nº de oportunidades y tasa de cierre) y responde las tres preguntas del lunes comercial — ① ¿Qué oportunidades puedo cerrar? (los cierres de la semana y las más valiosas en curso), ② ¿Qué operaciones están bloqueadas? (negocios estancados) y ③ ¿Qué acción generará ventas futuras? (tu actividad de los últimos 7 días y lo que tienes agendado). Trae un botón directo al Panel del Lunes en Reportes (ver 9.11)."),
        nota("El Resumen del Lunes usa el mismo interruptor de \"Notificaciones automáticas por email\" (ver 10.1.1): si los correos automáticos están activos, se envía; si están apagados, no. Solo llega a quien tenga pipeline o actividad que resumir esa semana."),
        h2("5.5 Llevar la agenda a tu calendario (Google, Outlook, Apple)"),
        p("Tienes tres formas de llevar tus actividades a tu calendario personal, según lo que necesites:"),
        p("Opción 1 — Una sola actividad: cada actividad de la lista tiene un botón Calendario que ofrece Google Calendar, Outlook o descargar un .ics con esa única actividad. Google y Outlook abren su pantalla de evento nuevo ya rellenada, donde eliges en qué calendario guardarla antes de confirmar."),
        p("Opción 2 — Un bloque (.ics): el botón iCal de la parte superior descarga en un archivo las actividades del filtro que tengas activo —Pendientes, Todas, Vencidas o Asignadas a mí—. El propio botón indica qué bloque va a descargar (por ejemplo, \"iCal: Vencidas\"). Cada actividad se exporta con título, tipo, fecha y hora, cliente y oportunidad vinculada (duración por defecto: 1 hora). Es una fotografía del momento: descarga el .ics de nuevo cuando quieras volver a sincronizar."),
        p("Opción 3 — Suscripción en vivo (recomendada): desde Mi perfil, en la sección \"Agenda en tu calendario\", genera tu enlace privado de suscripción y pégalo en Google, Outlook o Apple. A diferencia de las anteriores, este enlace se mantiene actualizado solo: cuando agregas o cambias actividades, tu calendario las refresca cada pocas horas sin volver a importar nada."),
        tip("Si abres directamente un .ics con muchas actividades, Windows lo importa al calendario por defecto sin preguntarte cuál. Para escoger el calendario de destino usa el botón Calendario de cada actividad (opción 1) o la suscripción (opción 3); si prefieres el archivo, en Google Calendar impórtalo desde Configuración > Importar y exportar, donde sí aparece el selector \"Añadir al calendario\"."),
        ...
        p("El botón \"Enviar email\" abre un panel con un campo de correo editable, pre-llenado con el email del contacto vinculado pero que puedes cambiar por cualquier otra dirección. El correo que recibe el cliente trae dos botones que funcionan sin iniciar sesión: \"Ver y responder en línea\" (abre la cotización, donde puede aceptarla o rechazarla) y \"Descargar PDF\", ambos por el enlace seguro de la cotización. También puedes generar un \"Link cliente\": esa misma URL pública de solo lectura para compartir por WhatsApp o mensaje directo, sin enviar correo."),
        nota("Si un cliente ve \"No autorizado\" al abrir el PDF, recibió una cotización enviada antes de esta mejora: vuelve a enviársela para que le llegue el enlace nuevo."),
        p("El cliente recibe el correo a nombre de tu empresa (con tu logo); \"Evoluteca CRM\" solo aparece como una nota pequeña al pie. Cuando el cliente responde, su respuesta se guarda automáticamente en el CRM: aparece en la tarjeta \"Correos\" de la cotización (ficha del contacto y de su oportunidad) y te llega un aviso a tu correo. El \"Correo de la empresa\" (ver 10.1) queda como respaldo del buzón de respuesta."),
        ...
        ...paso(3, "Fecha de creación (opcional)", "En el mapeo hay un campo \"Fecha de creación de estos clientes\". Déjalo vacío para clientes nuevos (usa la fecha de hoy); si cargas tu base histórica, elige una fecha pasada para que no cuenten como nuevos del año en curso en el filtro de clientes."),
        ...paso(4, "Confirmar importación", "Los tres campos obligatorios deben estar mapeados. Haz clic en Importar."),
        ...paso(5, "Resultado", "El sistema muestra cuántos registros se crearon. Verifica en Clientes y Contactos."),
        ...
        p("En \"Correo de la empresa\" (solo Administrador) guardas el correo de tu organización. Es un respaldo del buzón de respuesta de las cotizaciones: normalmente las respuestas del cliente se capturan solas en el CRM (ver 6.8), y este correo solo se usaría si la captura automática no estuviera disponible. Déjalo vacío si no quieres usarlo."),
        nota("El correo de cotización sale desde el dominio del sistema con el nombre de tu empresa. Enviarlo desde tu propio dominio (ej. cotizaciones@tuempresa.com) es un paso adicional que requiere verificar ese dominio; solicítalo si lo necesitas."),
        h3("10.1.1 Notificaciones automáticas por email"),
        p("El toggle \"Notificaciones automáticas por email\" controla los correos automáticos del CRM: las alertas diarias (actividades vencidas, negocios estancados y cierres próximos, ver 5.4) y el Resumen del Lunes semanal (ver 5.4.1). Cuando está activo (azul), cada usuario del equipo los recibe; las alertas cada mañana y el resumen cada lunes. Solo el Administrador puede cambiarlo; al desactivarlo, ningún usuario de la empresa recibe ninguno de los dos."),
        ...
        h2("10.6 Alerta de negocios estancados"),
        p("En Configuración, sección \"Alerta de negocios estancados\", el Administrador define desde cuántos días SIN MOVIMIENTO (sin actividad registrada ni cambio de etapa) una oportunidad activa se marca como estancada (por defecto 14). Ese umbral controla por igual el color de las tarjetas del Pipeline (ver 4.4) y la lista de estancados del correo diario (ver 5.4): al llegar al umbral es ámbar, al doble rojo. Ajústalo al ciclo de venta: más corto para ventas rápidas, más largo para ventas de varios meses."),
        h2("10.7 Campos personalizados"),
        p("En Configuración, sección \"Campos personalizados\", el Administrador crea campos propios (por ejemplo NIT, competidor o número de licitación) por entidad: Cliente u Oportunidad. Elige la entidad, ponle nombre y tipo (Texto, Número, Fecha, Lista de opciones fijas o Sí/No) y márcalo como obligatorio si aplica. El campo aparece en el formulario de edición de cada ficha y su valor se muestra en el detalle, en \"Datos personalizados\". Se pueden reordenar y eliminar; el tipo no se cambia una vez creado (para no invalidar datos guardados)."),
        h2("10.8 Automatizaciones"),
        p("En Configuración, sección \"Automatizaciones\", el Administrador crea reglas \"cuando pasa X → haz Y\" que se ejecutan solas. Evento: una oportunidad cambia de etapa (con filtro por etapa destino) o se crea una oportunidad (incluye los leads de la Captura externa). Acción: crear una tarea de seguimiento (título, plazo en días, responsable) o enviar un correo (al dueño del negocio o a los gerentes). En los textos puedes usar los marcadores {oportunidad} y {cliente}. Cada regla se activa/desactiva y muestra cuántas veces se ha ejecutado."),
        h2("10.9 Mi perfil"),
