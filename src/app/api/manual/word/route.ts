  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          children: [new TextRun({ text: `Versión 1.17  ·  ${new Date().toLocaleDateString("es-CO", { month: "long", year: "numeric" })}`, size: 20, color: GRIS, italics: true })],
          alignment: AlignmentType.CENTER,
          spacing: { after: 200 },
        }),
        new Paragraph({
        ...
        p("En la ficha de cada cliente está el panel \"Resumen con IA\". Al hacer clic en \"Generar resumen\", el CRM analiza toda la cuenta (contactos, oportunidades, actividades y cotizaciones) y redacta en segundos un resumen breve y accionable, que aparece a medida que se escribe. Siempre trae tres secciones:"),
        bullet("Estado de la relación — antigüedad, nivel de actividad y valor en juego"),
        bullet("Señales — riesgos (inactividad, cotizaciones sin cerrar, negocios perdidos) u oportunidades (negocios calientes, recompra)"),
        bullet("Próxima acción — la gestión concreta más recomendable ahora para ese cliente"),
        nota("Los Resúmenes con IA dependen del plan de tu organización: pueden ser ilimitados o tener un tope mensual (visible en el mismo panel como \"12 / 100 este mes\"). Si tu plan no los incluye, o si alcanzaste el tope del mes, el botón se deshabilita con el aviso correspondiente."),
        ...
        h2("5.5 Exportar al calendario (iCal)"),
        p("El botón iCal en la parte superior de la Agenda descarga todas tus actividades pendientes como archivo .ics, compatible con Google Calendar, Apple Calendar y Microsoft Outlook. Cada actividad se exporta con título, tipo, fecha y hora, cliente y oportunidad vinculada (duración por defecto: 1 hora)."),
        nota("Es una fotografía de tus actividades pendientes en ese momento, no una sincronización automática en tiempo real: descarga el .ics de nuevo cuando quieras volver a sincronizar."),
        ...
        h2("¿Qué es el Resumen con IA y qué datos usa?"),
        p("Es un resumen breve y accionable de una cuenta que genera la inteligencia artificial en la ficha del cliente (ver 3.10). Solo usa los datos reales de ese cliente dentro de tu CRM (contactos, oportunidades, actividades y cotizaciones) — no consulta información externa ni comparte tus datos con otras organizaciones. Según tu plan puede ser ilimitado o tener un tope mensual, visible en el mismo panel."),
