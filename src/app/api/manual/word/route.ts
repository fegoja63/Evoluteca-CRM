  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          children: [new TextRun({ text: `Versión 1.2  ·  ${new Date().toLocaleDateString("es-CO", { month: "long", year: "numeric" })}`, size: 20, color: GRIS, italics: true })],
          alignment: AlignmentType.CENTER,
          spacing: { after: 200 },
        }),
        new Paragraph({
        ...
        p("Al mover una oportunidad a la etapa Perdida, debes elegir un motivo de una lista (Presupuesto insuficiente, Eligió a la competencia, Sin respuesta del cliente, Precio, etc.). El motivo queda guardado en la ficha de la oportunidad y alimenta el reporte de Motivos de pérdida en Reportes (ver 9.3)."),
        pageBreak(),
        ...
        p("En ⚙️ Configuración puedes cambiar el nombre de tu empresa (aparece en el sidebar y en los PDFs de cotizaciones), y subir el logo de tu empresa desde un archivo de imagen (PNG o JPG) — aparece automáticamente en el encabezado de tus cotizaciones en PDF."),
