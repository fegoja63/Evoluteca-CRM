...
export async function GET() {
  ...
  const doc = React.createElement(Document,
    ...
    React.createElement(Page, { size: "A4", style: s.page },
      ...
      React.createElement(Nota, null, "Clientes es el único lugar donde se crea un cliente. Al guardarlo, el CRM te lleva directo al Pipeline a crear su oportunidad (etapa Prospecto), con el cliente y su contacto ya seleccionados: solo pones el título y el valor. Así cada cliente nuevo entra al pipeline desde el primer momento. Si solo querías registrar el cliente, puedes cerrar esa oportunidad sin guardarla."),
    ...
    React.createElement(Page, { size: "A4", style: s.page },
      ...
      React.createElement(P, null, 'También puedes crearla directamente en el Pipeline con el botón "Nueva oportunidad": elige la Empresa (cliente) de la lista y, si hace falta, un Contacto nuevo con "+ Nuevo"; al guardar se crean el contacto y la oportunidad, ya vinculados. El cliente debe existir: los clientes se crean solo en la pantalla Clientes. Si el cliente aún no está, usa el enlace "Créalo en Clientes" y, al guardarlo allá, el CRM te devuelve aquí con ese cliente ya seleccionado para crear su oportunidad.'),
    ...
    React.createElement(Page, { size: "A4", style: s.page },
      ...
      React.createElement(P, null, 'El cliente recibe el correo a nombre de tu empresa (con tu logo); "Evoluteca CRM" solo aparece como una nota pequeña al pie. Cuando el cliente responde, su respuesta se guarda automáticamente en el CRM: aparece en la tarjeta "Correos" de la cotización (ficha del contacto y de su oportunidad) y te llega un aviso a tu correo con un enlace para abrirla. El "Correo de la empresa" (ver 9.1) queda como respaldo del buzón de respuesta.'),
    ...
    React.createElement(Page, { size: "A4", style: s.page },
      ...
      React.createElement(LI, null, "Tablero de postventa: funciona como el Pipeline. Tiene cuatro etapas fijas (Entrega, Seguimiento, Renovación y Cerrado) y las tarjetas se arrastran entre columnas. Arriba muestra cuatro indicadores: valor en postventa, por renovar en 30 días, renovaciones creadas y cerrados. Además tiene buscador, filtro por vendedor (Gerente y Administrador) y cambio entre vista Kanban y Tabla; la tabla se ordena por negocio, cliente, etapa, valor o fecha de renovación. Un Comercial ve solo sus negocios."),
  ...
  return new NextResponse(buffer as unknown as BodyInit, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="manual-evoluteca-crm-${new Date().toLocaleString("sv-SE", { timeZone: "America/Bogota" }).slice(0, 16).replace(" ", "-").replace(":", "")}.pdf"`,
    },
  });
}
