...
export async function POST(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const cot = await prisma.cotizacion.findFirst({
    ...
    include: {
      ...
      tenant:   { select: { logoUrl: true, colorMarca: true, nombre: true, email: true } },
    },
  });
  ...
  const tablaFijo = `
    <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
      ...
      <tfoot>
        ...
        <tr style="background:#f8fafc"><td colspan="3" style="padding:12px;font-size:13px;font-weight:700;color:#1e293b">TOTAL</td><td style="padding:12px;font-size:15px;font-weight:700;color:${marca.oscuro};text-align:right">${fmt(total)}</td></tr>
      </tfoot>
    </table>`;
  ...
  const tablaSuccess = `
    <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
      ...
      <tfoot>
        ...
        <tr style="background:#f8fafc"><td colspan="2" style="padding:12px;font-size:13px;font-weight:700;color:#1e293b">HONORARIO ESTIMADO</td><td style="padding:12px;font-size:15px;font-weight:700;color:${marca.oscuro};text-align:right">${fmt(valorContrato)}</td></tr>
      </tfoot>
    </table>
  ...
  const tablaMensual = `
    <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
      <tbody>
        ...
        <tr style="background:#f8fafc"><td style="padding:12px;font-size:13px;font-weight:700;color:#1e293b">TOTAL DEL CONTRATO</td><td style="padding:12px;font-size:15px;font-weight:700;color:${marca.oscuro};text-align:right">${fmt(valorContrato)}</td></tr>
      </tbody>
    </table>`;
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#1e293b">
      <div style="background:${marca.oscuro};padding:24px 28px;border-radius:12px 12px 0 0;display:flex;align-items:center;justify-content:space-between">
        <div>
          <h2 style="color:white;margin:0;font-size:20px">${tenantNombre}</h2>
          <p style="color:${marca.sobreOscuro};margin:4px 0 0;font-size:13px">Cotización ${numero}</p>
        </div>
      ...
      <div style="background:white;padding:28px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px">
        ...
        <div style="margin-top:24px;text-align:center">
          <a href="${verUrl}" style="display:inline-block;background:${marca.oscuro};color:white;padding:12px 28px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600;margin:0 4px 8px">
            Ver y responder en línea
          </a>
          <a href="${pdfUrl}" style="display:inline-block;background:${marca.texto};color:white;padding:12px 28px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600;margin:0 4px 8px">
            ⬇ Descargar PDF
          </a>
        </div>
